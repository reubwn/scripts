...
while (<$BED>) {
  ...
  open($SAM, "bedtools intersect -sorted -g $genome -a $bam -b <(printf '$_\n') | samtools view - |");#`bedtools intersect -sorted -g $genome -a $bam -b <(printf "$_") | samtools view - | perl -lane 'if($F[6]eq"="){if($F[8]>500){$insert++};$same++;$total++}else{$total++}END{print "$total\t$same\t".($same/$total)."\t$insert\t".($insert/$total)}'`;
  while (<$SAM>) {
    my @F = split (/\s+/, $_);
